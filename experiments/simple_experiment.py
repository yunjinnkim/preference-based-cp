...
from mapie.conformity_scores import NaiveConformityScore, LACConformityScore, APSConformityScore, RAPSConformityScore, TopKConformityScore
...
def evaluate_predictions(y_test, y_pred_crisp, y_pred_set, conformity_score_name, alpha_value, clf_seed, mccv_split_seed,gradient_updates, result_processor):
...
def worker(parameters: dict, result_processor: ResultProcessor, custom_config: dict):
    ...
    match parameters["model"]:
        ...
        case "classifier":
            ...
            conformity_scores = [NaiveConformityScore(), LACConformityScore(), APSConformityScore(), RAPSConformityScore(), TopKConformityScore()]
            for conformity_score, alpha in product(conformity_scores, alphas):            
                predictor = MapieClassifier(estimator=model, cv="prefit", conformity_score=APSConformityScore())
                ...
                y_pred_set = [np.where(prediction_set_row)[0].tolist() for prediction_set_row in y_pred_set]

                conformity_score_name = type(conformity_score).__name__
                evaluate_predictions(y_test, y_pred_crisp, y_pred_set, conformity_score_name, alpha, clf_seed, mccv_split_seed,gradient_updates, result_processor)
...
if __name__ == "__main__":
    ...
    experimenter = PyExperimenter(
        experiment_configuration_file_path="./experiments/config/cfg_simple_debug.yml", use_codecarbon=False
    )
